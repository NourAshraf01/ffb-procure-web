...
class SCMProjSeminarSupabaseUser extends BaseAuthUser {
  SCMProjSeminarSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> sCMProjSeminarSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SCMProjSeminarSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
